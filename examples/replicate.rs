//! adapted from hypercore protocol docs example
//! https://docs.rs/hypercore-protocol/latest/hypercore_protocol/#example
use async_std::prelude::*;
use hypercore_protocol::schema::*;
use hypercore_protocol::{Event, Message, ProtocolBuilder};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    // Start a tcp server.
    println!("satrting listener");
    let listener = async_std::net::TcpListener::bind("localhost:8000")
        .await
        .unwrap();
    println!("listening");
    ...
    // Connect a client.
    println!("satrting dialer");
    let stream = async_std::net::TcpStream::connect("localhost:8000")
        .await
        .unwrap();
    println!("dialing");
