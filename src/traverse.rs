...
pub struct TraverseNodes<M: CoreMem> {
    root: SharedNode<M>,
    /// Option holding (number_of_keys, number_of_children) for this node
    n_keys_and_children: Option<PinnedFut<(usize, usize)>>,
    ...
    iter: Option<Box<dyn Iterator<Item = usize>>>,
    /// Future holding the next child
    next_child: Option<PinnedFut<Result<TraverseNodes<M>, HyperbeeError>>>,
    /// Another instance of [`TraverseNodes`] from a child node.
    child_stream: Option<Pin<Box<TraverseNodes<M>>>>,
}

/// Result<SharedNode>
type TraverseNodesItem<M> = Result<SharedNode<M>, HyperbeeError>;

impl<M: CoreMem> TraverseNodes<M> {
    fn new(root: SharedNode<M>) -> Self {
        TraverseNodes {
            root,
            n_keys_and_children: Option::None,
...
async fn get_child_node_stream<M: CoreMem>(
    node: SharedNode<M>,
    index: usize,
) -> Result<TraverseNodes<M>, HyperbeeError> {
    let child = node.read().await.get_child(index).await?;
    Ok(TraverseNodes::new(child))
}

impl<M: CoreMem + 'static> Stream for TraverseNodes<M> {
    type Item = TraverseNodesItem<M>;
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        let iter = match &mut self.iter {
            None => {
                match &mut self.n_keys_and_children {
                    ...
                    Some(fut) => match fut.poll(cx) {
                        Poll::Ready((_, n_children)) => {
                            let iter: Box<dyn Iterator<Item = usize>> = if n_children != 0 {
                                Box::new(0..n_children)
                            } else {
                                Box::new(0..0)
                            };
                            self.iter = Some(iter);
                            if n_children == 0 {
                                return Poll::Ready(Some(Ok(self.root.clone())));
                            }
                        }
                        Poll::Pending => (),
                    },
        ...
        if let Some(index) = iter.next() {
            self.next_child = Some(Box::pin(get_child_node_stream(self.root.clone(), index)));
            // TODO is this needed?
            cx.waker().wake_by_ref();
            return Poll::Pending;
        }
...
pub fn traverse_nodes<M: CoreMem>(node: SharedNode<M>) -> TraverseNodes<M> {
    TraverseNodes::new(node)
}
